...
let lastWindowState = chrome.windows.WindowState.NORMAL;
...
async function toggleFullscreen(state?: 'on' | 'off') {
	...
	if (isTankTrouble(tab.url)) {
		...
		lastWindowState = currentState as chrome.windows.WindowState; // eslint-disable-line require-atomic-updates
	}
}
/**
